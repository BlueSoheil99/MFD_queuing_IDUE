...
figure(2)
plot(n_data(1,:),'LineWidth',1.5,'LineStyle','-.')
hold on
for ix = 2:num_reg
    if ix <= 12
        if mod(ix,2) == 1
            plot(n_data(ix,:),'LineWidth',1.5,'LineStyle','-.')
        else
            plot(n_data(ix,:),'LineWidth',1.5,'LineStyle','--')
        end
    else
        plot(n_data(ix,:),'LineWidth',1.5)
    end
end
hold off
...
for t = 1:N
    ...
    plot_qD(t) = q_data_all(communi,t);
end
hold on
yyaxis left
...
legend(['Buffer zone inflow via link (' num2str(in) ',' num2str(out) ')'], ['Buffer zone outflow via link (' num2str(in) ',' num2str(out) ')'],... %'Flow capacity of link (13,19)',...
        ['Buffer zone qD via link (' num2str(in) ',' num2str(out) ')'])
ylim([0,1]);
yticks(0:0.5:1);
xlim([0,N]);
hold off
