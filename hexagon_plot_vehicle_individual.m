...
t_matrix = [1 500 1000 1500 2000 3000 4000 5000];
for i=1:1:length(t_matrix)
    ...
    x = cos(t);
    y = sin(t);
    color = a(ceil(n_data(19,tt)/n_max*64),:);
    fill(x,y,color)
    axis image
...
%     colorbar
    ...
    x = cos(t)+1.5;
    y = sin(t)+sqrt(3)/2;
    color = a(ceil(n_data(15,tt)/n_max*64),:);
    fill(x,y,color)

    % region 16
    x = cos(t);
    y = sin(t)+sqrt(3);
    color = a(ceil(n_data(16,tt)/n_max*64),:);
    fill(x,y,color)

    % region 17
    x = cos(t)-1.5;
    y = sin(t)+sqrt(3)/2;
    color = a(ceil(n_data(17,tt)/n_max*64),:);
    fill(x,y,color)

    % region 18
    x = cos(t)-1.5;
    y = sin(t)-sqrt(3)/2;
    color = a(ceil(n_data(18,tt)/n_max*64),:);
    fill(x,y,color)

    % region 13
    x = cos(t);
    y = sin(t)-sqrt(3);
    color = a(ceil(n_data(13,tt)/n_max*64),:);
    fill(x,y,color)

    % region 14
    x = cos(t)+1.5;
    y = sin(t)-0.5*sqrt(3);
    color = a(ceil(n_data(14,tt)/n_max*64),:);
    fill(x,y,color)

    % region 1
    x = cos(t);
    y = sin(t)-2*sqrt(3);
    color = a(ceil(n_data(1,tt)/n_max*64),:);
    fill(x,y,color)

    % region 2
    x = cos(t)+1.5;
    y = sin(t)-1.5*sqrt(3);
    color = a(ceil(n_data(2,tt)/n_max*64),:);
    fill(x,y,color)

    % region 3
    x = cos(t)+3;
    y = sin(t)-sqrt(3);
    color = a(ceil(n_data(3,tt)/n_max*64),:);
    fill(x,y,color)

    % region 4
    x = cos(t)+3;
    y = sin(t);
    color = a(ceil(n_data(4,tt)/n_max*64),:);
    fill(x,y,color)

    % region 5
    x = cos(t)+3;
    y = sin(t)+sqrt(3);
    color = a(ceil(n_data(5,tt)/n_max*64),:);
    fill(x,y,color)

    % region 6
    x = cos(t)+1.5;
    y = sin(t)+1.5*sqrt(3);
    color = a(ceil(n_data(6,tt)/n_max*64),:);
    fill(x,y,color)

    % region 7
    x = cos(t);
    y = sin(t)+2*sqrt(3);
    color = a(ceil(n_data(7,tt)/n_max*64),:);
    fill(x,y,color)

    % region 8
    x = cos(t)-1.5;
    y = sin(t)+1.5*sqrt(3);
    color = a(ceil(n_data(8,tt)/n_max*64),:);
    fill(x,y,color)

    % region 9
    x = cos(t)-3;
    y = sin(t)+sqrt(3);
    color = a(ceil(n_data(9,tt)/n_max*64),:);
    fill(x,y,color)

    % region 10
    x = cos(t)-3;
    y = sin(t);
    color = a(ceil(n_data(10,tt)/n_max*64),:);
    fill(x,y,color)

    % region 11
    x = cos(t)-3;
    y = sin(t)-sqrt(3);
    color = a(ceil(n_data(11,tt)/n_max*64),:);
    fill(x,y,color)

    % region 11
    x = cos(t)-1.5;
    y = sin(t)-1.5*sqrt(3);
    color = a(ceil(n_data(12,tt)/n_max*64),:);
    fill(x,y,color)
    ...
    for i =1:1:size(region_communi, 1)
        if p_data(i,19,tt) > 0
            line([center(region_communi(i,1),1),center(region_communi(i,2),1)],[center(region_communi(i,1),2),center(region_communi(i,2),2)],'Color','k', 'LineWidth',3);
        else
        end
    end
