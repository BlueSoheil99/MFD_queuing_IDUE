...
def generate_demand_mat(net_edges_and_labels:dict, vehroute_xml, out_adr='output/pq_input/demand.mat'):
    # 1- MAKE EMPTY NUMPY ARRAY
    # import pdb
    # pdb.set_trace()
    n_regions = len(np.unique(np.array(list(net_edges_and_labels.values()))))
    time = 180000  # todo change
    matrix = np.zeros((n_regions, n_regions, time))
