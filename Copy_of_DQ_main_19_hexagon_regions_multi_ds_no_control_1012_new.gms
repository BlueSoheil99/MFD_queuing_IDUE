...
$include Copy_of_DQ_network_19_hexagon_regions_multi_ds_no_control_1012_new.gms

*variables
nonnegative variable
         theta(i,j,d) 'the queue inflow of the boundary (i,j)',
         tau0(i,d) 'the minimum travel time from region i to destination',
         y(i,j) 'auxiliary variable for Cbar(i,j)',
         lamda(i,j) 'auxiliary variable for Cbar(i,j)',
         mu(i,j) 'auxiliary variable for Cbar(i,j)' ;
...
* the DCS formulation
theta_com(i,j,d)$region_communi(i,j) ..          MFD_Para(i, 'length')*sum(d2, n(i,d2))/(MFD_Para(i, 'm3')*power(sum(d2, n(i,d2)),3)+MFD_Para(i, 'm2')*power(sum(d2, n(i,d2)),2)+MFD_Para(i, 'm1')*sum(d2, n(i,d2)) + MFD_Para(i, 'm0'))+ sum(d2, q(i,j,d2))/(y(i,j)*(MFD_Para(j, 'n_bar')-sum(d2, n(j,d2)))+0.1)+tau0(j,d)-tau0(i,d) =g= 0;
...
lamda_com(i,j)$region_communi(i,j)..             Cbar(i,j)-y(i,j)*(MFD_Para(j, 'n_bar')-sum(d2, n(j,d2))) =g= 0;
mu_com(i,j)$region_communi(i,j)..                y(i,j)*(MFD_Para(j, 'n_bar')-sum(d2, n(j,d2))) =g= 0;

tau0.fx(i,d)$(sameas(i,d))=0;

* solve the problem
model mfd / theta_com.theta, tau0_com.tau0, y_com.y, lamda_com.lamda, mu_com.mu /;
solve mfd using mcp
