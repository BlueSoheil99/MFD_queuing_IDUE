...
* the DCS formulation
theta_com(i,j,d)$region_communi(i,j) ..          MFD_Para(i, 'length')*sum(d2, n(i,d2))/(MFD_Para(i, 'm3')*power(sum(d2, n(i,d2)),3)+MFD_Para(i, 'm2')*power(sum(d2, n(i,d2)),2)+MFD_Para(i, 'm1')*sum(d2, n(i,d2)) + MFD_Para(i, 'm0')) + sum(d2, q(i,j,d2))/(u(i,j)+0.1) + tau0(j,d) - tau0(i,d) =g= 0;
