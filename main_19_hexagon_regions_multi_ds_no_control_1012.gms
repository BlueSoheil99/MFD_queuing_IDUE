...
nonnegative variable
         p(i,j,d) 'the queue inflow of the boundary (i,j)',
...
* the DCS formulation
p_com(i,j,d)$region_communi(i,j) ..              MFD_Para(j, 'length')*sum(d2, n(j,d2))/(MFD_Para(j, 'm3')*power(sum(d2, n(j,d2)),3)+MFD_Para(j, 'm2')*power(sum(d2, n(j,d2)),2)+MFD_Para(j, 'm1')*sum(d2, n(j,d2)) + MFD_Para(j, 'm0'))+ sum(d2, q(i,j,d2))/(y(i,j)*(MFD_Para(j, 'n_bar')-sum(d2, n(j,d2)))+0.0001)+eta(j,d)-eta(i,d) =g= 0;
eta_com(i,d)..                                   sum(region_communi(i,j), p(i,j,d))-(MFD_Para(i, 'm3')*power(sum(d2, n(i,d2)),3)+MFD_Para(i, 'm2')*power(sum(d2, n(i,d2)),2)+MFD_Para(i, 'm1')*sum(d2, n(i,d2)) + MFD_Para(i, 'm0'))/sum(d2, n(i,d2))*n(i,d)/MFD_Para(i, 'length') =g= 0;
